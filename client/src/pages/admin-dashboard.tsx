...
const NAV_VIEWS: { view: AdminView; label: string; icon: React.ReactNode }[] = [
  ...
  { view: "users",     label: "Users",     icon: <Users className="w-3.5 h-3.5" /> },
  { view: "verify",    label: "Verify",    icon: <ShieldCheck className="w-3.5 h-3.5" /> },
  { view: "roles",     label: "Hierarchy", icon: <ShieldAlert className="w-3.5 h-3.5" /> },
  { view: "registry",  label: "Registry",  icon: <Globe className="w-3.5 h-3.5" /> },
  { view: "payouts",   label: "Ledger",    icon: <CreditCard className="w-3.5 h-3.5" /> },
  { view: "system",    label: "System",    icon: <LayoutDashboard className="w-3.5 h-3.5" /> },
];
...
export default function AdminDashboard() {
  ...
  // Prev / Next through the main nav views only (excludes settings/account)
  const mainViews = NAV_VIEWS;
  const currentIdx = mainViews.findIndex(v => v.view === view);
  const prevNav = currentIdx > 0 ? mainViews[currentIdx - 1] : null;
  const nextNav = currentIdx < mainViews.length - 1 && currentIdx >= 0 ? mainViews[currentIdx + 1] : null;
  const currentNav = mainViews[currentIdx] ?? mainViews[0];

  const renderContent = () => {
    switch (view) {
      case "users":    return <AdminUsersView />;
      case "verify":   return <AdminVerifyView metrics={safeMetrics} />;
      case "registry": return <AdminRegistryView />;
      case "roles":    return <AdminRolesView />;
      case "payouts":  return <AdminPayoutsView />;
      case "settings": return <AdminSettingsView />;
      case "account":  return <AdminAccountView onLogout={logout} />;
      case "system":   return <AdminSystemView metrics={safeMetrics} />;
      default:         return <AdminOverview metrics={safeMetrics} aiInsights={aiInsights} onNavigate={setView} />;
    }
  };
      ...
      <header className="border-b border-neutral-200/50 bg-white/80 backdrop-blur-2xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-8">
            {/* Logo — always returns to Pulse overview */}
            <button
            ...
            {/* Desktop nav — all client-side state, no URL changes */}
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={cn(
                ...
                view === "settings" ? "border-primary ring-2 ring-primary/20" : "border-neutral-200"
              )}
            ...
            <button
              ...
              className={cn(
                ...
                view === "account" ? "bg-primary/80 text-white" : "bg-primary text-white shadow-primary/20"
              )}
              title="My Account"
            >
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-10 flex items-center justify-between border-t border-neutral-100">
          <div className="flex items-center gap-2 text-[11px] font-bold text-neutral-400">
            <button onClick={() => setView("overview")} className="hover:text-primary transition-colors flex items-center gap-1">
            ...
            {view !== "overview" && (
              <>
                ...
                <span className="text-neutral-700 flex items-center gap-1">
                  {currentNav?.icon} {currentNav?.label ?? view}
                </span>
              </>
            )}
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => prevNav && setView(prevNav.view)}
              disabled={!prevNav || view === "settings" || view === "account"}
              className={cn(
                "flex items-center gap-1.5 px-3 py-1 rounded-lg text-[11px] font-bold transition-all border",
                prevNav && view !== "settings" && view !== "account"
            ...
            <button
              onClick={() => nextNav && setView(nextNav.view)}
              disabled={!nextNav || view === "settings" || view === "account"}
              className={cn(
                "flex items-center gap-1.5 px-3 py-1 rounded-lg text-[11px] font-bold transition-all border",
                nextNav && view !== "settings" && view !== "account"
                  ? "bg-white hover:bg-neutral-50 text-neutral-600 border-neutral-200 hover:border-primary/30"
                  : "opacity-30 cursor-not-allowed bg-neutral-50 text-neutral-400 border-neutral-100"
              )}
            >
...
function AdminOverview({ metrics, aiInsights, onNavigate }: { metrics: AdminMetrics; aiInsights: any; onNavigate?: (v: AdminView) => void }) {
  const [, setLocation] = useLocation();
